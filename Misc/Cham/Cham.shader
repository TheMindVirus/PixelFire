Shader "PixelFire/Cham"
{
    ...
    SubShader
    {
        ...
        void surface(Input input, inout SurfaceOutputStandard output)
        {
            ...
            output.Metallic = 0.0; //_Metallic;
            output.Smoothness = 0.0; //_Glossiness;

            //output.Normal = -input.normal; //output.Normal; // = input.normal; //1.0;
            output.Emission = input.normal; //0.0; //fragment.rgb;
            output.Occlusion = 1.0; //0.5;

            output.Albedo = output.Emission;
        }
/*
